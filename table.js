...
const update_table = async function (table, values, names_arr) {
	...
	for (const value of values) {
		...
		for (let i = 0; i < names_arr.length; i++) {
			...
			//Create text
			if(i === 4) {
				new_cell.innerHTML = '<a class = "link_target_chembl" href="https://www.ebi.ac.uk/chembl/explore/target/' + value[names_arr[i]].replace(/_.*/, "") + '" target = "_blank">' + value[names_arr[i]] + '</a>';
			} else {
