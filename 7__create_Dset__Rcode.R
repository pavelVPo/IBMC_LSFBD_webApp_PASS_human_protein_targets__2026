...
structs 	<- read_tsv("C:/.../structs.tsv") |> mutate(molfile = str_replace_all(molfile, "\n\r\n", "\r\n")) |> mutate(molfile = str_replace_all(molfile, "\n \r\n", "\r\n"))
...
activities_all <- activities_a_m_c |> select(tid, molregno, p_act) |>
										distinct() |>
										inner_join(cutoffs) |>
										mutate(activity = if_else(p_act >= cutoff, str_glue('target_{tid}'), str_glue('!target_{tid}')), activity_n = if_else(p_act >= cutoff, 1, 0))
